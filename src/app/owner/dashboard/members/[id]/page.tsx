...
export default function MemberDetailPage() {
  ...
  const totalPaid = (recentPayments ?? [])
    ...
    .reduce((sum: number, p: any) => sum + (p.amount || 0), 0) / 100; // Convert cents to currency
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="flex items-center justify-end gap-3 mt-5 pt-4 border-t border-gray-100">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-[#E8541A] text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-[#E8541A]/90 transition-colors shadow-sm"
          >
            <CreditCard size={16} />
            Record Payment
          </button>
                ...
                <tbody className="divide-y divide-gray-100">
                  {recentPayments.map((p: any, i: number) => (
                    <tr key={p._id} className={i % 2 !== 0 ? "bg-[#F9F9F9]" : "bg-white"}>
                      ...
                      <td className="px-6 py-3 text-sm text-gray-600 capitalize">{p.method || "—"}</td>
                      <td className="px-6 py-3">
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${
                          ...
                          {(p.status === "paid" || p.status === "completed") ? "Paid" : p.status}
                        </span>
                      </td>
